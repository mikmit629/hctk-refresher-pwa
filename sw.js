const CACHE_NAME = 'hctk-serverless-pwa-v69';
const APP_SHELL = [
  './',
  './index.html',
  './styles.css',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((key) => key.startsWith('hctk-serverless-pwa-') && key !== CACHE_NAME).map((key) => caches.delete(key)))
    )
  );
  self.clients.claim();
});
